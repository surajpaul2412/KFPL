...
class FormService
{
  ...
  public static function NumberintoWords(float $number)
  {
	  ...
	  while ($x < $count_length) {
		  ...
		  if ($number) {
			  $add_plural = (($counter = count($string)) && $number > 9) ? '' : null;
			  $amt_hundred = ($counter == 1 && $string[0]) ? ' and ' : null;
			  $string[] = ($number < 21) ? ( isset($change_words[$number]) ? $change_words[$number] : '' ) . ' ' . 
			              (isset($here_digits[$counter]) ? $here_digits[$counter] : '') . $add_plural . ' ' .
