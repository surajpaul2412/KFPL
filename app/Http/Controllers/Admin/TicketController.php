...
class TicketController extends Controller
{
     ...
     */
     ...
     {
        // SEARCH PArameters
        $sel_status_id = isset($request['sel_status_id']) ? $request['sel_status_id'] : '';
        $sel_from_date = isset($request['sel_from_date']) ? $request['sel_from_date'] : '';
        $sel_to_date = isset($request['sel_to_date']) ? $request['sel_to_date'] : '';
        $sel_role_id = isset($request['sel_role_id']) ? $request['sel_role_id'] : '';
        ...
        if($sel_status_id != '') $ticketQuery->where('status_id', $sel_status_id);
        if($sel_from_date != '') $ticketQuery->where('updated_at', '>=', $sel_from_date);
        if($sel_to_date != '') $ticketQuery->where('updated_at', '<=', $sel_to_date);
        if($sel_query != ''){
          $ticketQuery->whereHas( 'security', function (Builder $query) use ($sel_query) {
                                             $query->where('tickets.id', 'LIKE', "%{$sel_query}%")
        ...
        if( $sel_role_id != '')
        {
            $ticketQuery->whereHas('userroles', function ($query) use ($sel_role_id) {
                 $query->whereRaw('tickets.user_id = role_user.user_id and role_user.role_id = '. $sel_role_id);
            });
        }
