...
class MisController extends Controller
{
	...
	public function getMisData(Request $request)
    {
        ...
        if ($setType == 1) { // BUY case
            ...
            $quickTicketData = QuickTicket::with('user', 'role_user', 'security', 'security.amc')
					->selectRaw('
                        security_id, 
                    ...
                    ->whereHas("user", function ($query) use ($role_id) {
						$query->leftJoin("role_user", function($leftJoin) use ($role_id)
						{
							$leftJoin->on('users.id', '=', 'role_user.id')
            ...
            $ticketData = Ticket::with('user', 'role_user', 'security', 'security.amc')
					->selectRaw('
                        security_id, 
